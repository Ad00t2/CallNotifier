...
export function stringifyUri(uri) {
...
export function stringifyAuthHeader(a) {
  var s = [];
...
export function makeResponse(rq, status, reason, extension) {
  var rs = {
    status: status,
    reason: reason || '',
    version: rq.version,
...
export const copyMessage = function(msg, deep) {
...
export function makeStreamParser(onMessage) {
  var m;
  var r = '';
...
function makeTlsTransport(options, callback) {
  return makeStreamTransport(
    'TLS',
    (port, host, callback) => { return tls.connect(port, host, options.tls, callback); },
    (callback) => {
...
function makeTcpTransport(options, callback) {
  return makeStreamTransport(
    'TCP',
    (port, host, callback) => { return net.connect(port, host, callback); },
    (callback) => {
...
export function makeTransport(options, callback) {
  var protocols = {};
...
export function resolve(uri, action) {
  ...
  var calling = {
    enter: function() {
      ...
      b = setTimeout(function() {
        tu(makeResponse(rq, 408, '(Client sip.js:1018) TIMEOUT'));
        sm.enter(terminated);
      }, 32000);
    },
    leave: function() {
...
function createClientTransaction(rq, transport, tu, cleanup) {
  ...
  var trying = {
    ...
    timerF: function() {
      tu(makeResponse(rq, 408, '(CLIENT sip.js:1122) TIMEOUT'));
      sm.enter(terminated);
    }
  };
...
export function makeTransactionLayer(options, transport) {
...
function sequentialSearch(isTls, tlsAddress, transaction, connect, addresses, rq, callback) {
  if(rq.method !== 'CANCEL') {
  ...
  function next() {
    ...
    if(addresses.length > 0) {
      try {
        ...
        if (isTls) address.address = tlsAddress;
        var client = transaction(connect(address, function(err) {
          if(err) {
            console.log("err: ", err);
          }
          client.message(makeResponse(rq, 503, '(CLIENT sip.js:1219) SERVICE UNAVAILABLE'));
        }), rq, function() { onresponse.apply(null, arguments); });
      }
      catch(e) {
        onresponse(address.local ? makeResponse(rq, 430, '(CLIENT sip.js:1223) FLOW FAILED') : makeResponse(rq, 503, '(CLIENT sip.js:1223) SERVICE UNAVAILBLE'));
      }
    } else {
      onresponse = callback;
      onresponse(makeResponse(rq, lastStatusCode || 404, '(CLIENT sip.js:1227) NO ADDRESSES IN SEQUENTIALSEARCH'));
    }
  }
...
export function create(options, callback) {
  var errorLog = (options.logger && options.logger.error) || ((e) => { console.log(e); });

  var transport = makeTransport(options, (m, remote) => {
    try {
      var t = m.method ? transaction.getServer(m) : transaction.getClient(m);

      if (!t) {
        if (m.method && m.method !== 'ACK') {
          var t = transaction.createServerTransaction(m, transport.open(remote));
          try {
            callback(m, remote);
          } catch (e) {
            errorLog(e);
            t.send(makeResponse(m, '500', '(CLIENT sip.js:1257) INTERNAL ERROR'));
          }
        }
      ...
      if(m.method === undefined) {
        ...
        (function(callback) {
          ...
          if(m.method === 'ACK') {
            ...
            sequentialSearch(('tls' in options), options.tlsAddress, transaction.createClientTransaction.bind(transaction), transport.open.bind(transport), addresses, m, callback || function() {});
        });
      }
    },
    encodeFlowUri: (flow) => {
